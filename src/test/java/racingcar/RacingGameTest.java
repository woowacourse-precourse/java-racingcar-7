...
public class RacingGameTest {
    ...
    public void testCarNameDuplication() {
        ...
        game.cars.add(new Car("testCar"));

        // 중복된 자동차 이름이 있는 경우 오류발생
        assertThrows(IllegalArgumentException.class, () -> {
            game.checkDuplicateCar("testCar");
        }, "자동차 이름은 중복될 수 없습니다.");
    }
    ...
    public void testMaxTurnValidation() {
        ...
        // 이동횟수가 숫자가 아닐 때
        assertThrows(IllegalArgumentException.class, () -> {
            game.validateMaxTurn("twenty");
        }, "이동횟수는 숫자만 입력 가능합니다.");

        //이동횟수가 1보다 적을 때
        assertThrows(IllegalArgumentException.class, () -> {
            game.validateMaxTurn("0");
        }, "이동횟수는 최소 1 이상입니다.");


        // 범위 내 태스트
        assertDoesNotThrow(() -> {
            game.validateMaxTurn("1");
        }, "이동횟수가 최소값 1일 때 예외가 발생하지 않아야 합니다.");

        assertDoesNotThrow(() -> {
            game.validateMaxTurn("10");
        }, "이동횟수가 중간값 10일 때 예외가 발생하지 않아야 합니다.");

        assertDoesNotThrow(() -> {
            game.validateMaxTurn("20");
        }, "이동횟수가 최대값 20일 때 예외가 발생하지 않아야 합니다.");


        //이동횟수가 20보다 클 때
        assertThrows(IllegalArgumentException.class, () -> {
            game.validateMaxTurn("21");
        }, "이동횟수는 최대 20 이하입니다.");
