...
class CarsTest {
    ...
    void 정상적으로_승자를_판단하는가(int position1, int position2) {
        ...
        if (position1 == position2) {
            assertThat(cars.getWinners().get(0)).isEqualTo(car1.getDto());
            assertThat(cars.getWinners().get(1)).isEqualTo(car2.getDto());
            return;
        }
        if (position1 > position2) {
            assertThat(cars.getWinners().get(0)).isEqualTo(car1.getDto());
        ...
        assertThat(cars.getWinners().get(0)).isEqualTo(car2.getDto());
        assertThat(cars.getWinners().size()).isEqualTo(1);
    }
}
