...
class InputHandlerTest {
    ...
    @ParameterizedTest
    @CsvSource(value = {"'     '", "''", "''"})
    void 문자열_null이거나_비어있는_경우_예외처리(final String input) {
    ...
    void 중복된_이름이_있으면_예외처리(final String input) {
    ...
    void 숫자가_입력되지_않은_경우_예외처리(final String input) {
    ...
    void testCheckMinimumOne(final int input) {
        assertThatThrownBy(() -> inputHandler.checkMinimumOne(input)).isInstanceOf(IllegalArgumentException.class);
    }
}