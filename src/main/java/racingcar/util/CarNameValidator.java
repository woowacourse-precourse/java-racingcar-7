...
public class CarNameValidator {
    ...
    private static void checkInputIsEmpty(String input) {
        if (input == null || input.isBlank()) {
            throw new IllegalArgumentException(CarNameInputErrorMessages.CAR_NAME_IS_EMPTY.getMessage());
        }
    }
    ...
    private static void checkInputIncludeWhiteSpace(String input) {
        if (input.contains(" ")) {
            throw new IllegalArgumentException(CarNameInputErrorMessages.CAR_NAME_CONTAINS_WHITESPACE.getMessage());
        }
    }
    ...
    private static void checkInputIncludeInvalidDelimiter(String input) {
        if (!input.matches("^[a-zA-Z,]+$")) {
            throw new IllegalArgumentException(CarNameInputErrorMessages.INVALID_DELIMITER.getMessage());
        }
    }
    ...
    private static void checkNameLength(String carName) {
        if (carName.length() < 1 || carName.length() > 5) {
            throw new IllegalArgumentException(CarNameInputErrorMessages.CAR_NAME_TOO_LONG.getMessage());
        }
    }
