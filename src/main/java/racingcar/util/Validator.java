...
public class Validator {
    ...
    public RacingInfoDTO validInput(RacingInputReq racingInputReq) {
        ...
        // 아무 값도 입력되지 않은 경우
        if (validator.validBlank(racingInputReq.getCarNames(), racingInputReq.getRaceRounds())) {
            throw new IllegalArgumentException(ErrorMessage.EMPTY_ARG.getMessage());
        }

        Splitter separator = new Splitter(racingInputReq.getCarNames());

        // 시도할 횟수로 수가 아닌 값이 입력된 경우
        if (validator.validNumber(racingInputReq.getRaceRounds())) {
            throw new IllegalArgumentException(ErrorMessage.NON_NUMBER.getMessage());
        }

        RacingInfoDTO racingInfoDTO = new RacingInfoDTO(separator.splitSeparator(), new BigInteger(racingInputReq.getRaceRounds()));

        // 자동차 이름 길이가 5보다 큰 경우
        if (validator.validNameLength(racingInfoDTO.getCarList())) {
