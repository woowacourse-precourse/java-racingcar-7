...
public class InputValidator {
    ...
    private static final String CAR_NAME_REGEX = "^[a-zA-Z0-9,가-힣]+$";
    private static final String TRY_COUNT_REGEX = "^[0-9]+$";
    private static final String ERROR_INVALID_NAME_FORMAT = "적절하지 않은 이름 형식입니다.";
    private static final String ERROR_EXCEED_NAME_LENGTH = "이름의 제한 길이(5글자)를 초과하였습니다.";
    private static final String ERROR_DUPLICATE_NAMES = "중복되는 이름이 존재합니다.";
    ...
    public static int parseAndValidateTryCount(String tryCount) {
    ...
    private static void validateIsNumber(String tryCount) {
        if (!tryCount.matches(TRY_COUNT_REGEX)) {
            throw new IllegalArgumentException(ERROR_NOT_NUMBER);
        }
    }

    private static int parseAndValidateIntRange(String tryCount) {
        try {
    ...
    private static void validateIsNaturalNumber(int count) {
        if (count <= 0) {
