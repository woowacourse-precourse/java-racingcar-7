...
- indent(인덴트, 들여쓰기) depth를 3 이상으로 하지 않도록 하여, **최대 2단계의 들여쓰기**만 허용할 것입니다. 예를 들어, `while`문 안에 `if`문이 있을 때 들여쓰기는 2단계까지만 유지합니다.
...
- **세부 사항**:
  ...
  - 예) `5`
- **예외 발생**:
  - 만약 사용자가 0 또는 음수를 입력할 경우 `IllegalArgumentException`를 발생시킨다.
  - 예) `IllegalArgumentException`: "시도할 횟수는 양수여야 합니다."
- **유효성 검사**:
  - 유효하지 않은 입력일 경우 사용자에게 에러 메시지를 출력하고 다시 입력을 받도록 구현할 수 있다.
