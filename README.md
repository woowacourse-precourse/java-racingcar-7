...
- 자동차 이름
    ...
    - [ ] 자동차 이름의 양 옆 공백을 제거한다.(`trim()`)
    - [ ] 자동차 이름이 5자 이하인지 검증한다.

- 자동차 이동 시도 횟수
    - [ ] 자동차 이동 시도 횟수가 0 이상 양수인지 검증한다.
    - [ ] 자동차 이동 시도 횟수만큼 라운드를 반복한다.
...
- 경기 종료
    - [ ] 가장 많이 움직인 자동차(또는 자동차들)를 판단한다.
    - [ ] 경주가 끝난 후 우승자(또는 우승자들)를 출력한다.

### 입출력 구현 리스트

- 입력
    - [ ] 사용자로부터 자동차 이름 문자열을 입력받는다.
    - [ ] 사용자로부터 자동차 이동 시도 횟수를 입력받는다.

- 츨력
    - [ ] 자동차 이름 문자열 입력 문구를 출력한다. `경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)`
    - [ ] 자동차 이동 시도 횟수 문구를 출력한다. `시도할 횟수는 몇 회인가요?`
    - [ ] 자동차 실행 결과 문구를 출력한다. `실행 결과`
    - [ ] 라운드마다 자동차 이름과 이동 횟수 `'-'`를 출력한다. `pobi : -`
    - [ ] 최종 우승자 문구를 출력한다. `최종 우승자 :`
    - [ ] 최종 우승자 자동차 이름(또는 이름들)을 출력한다. `pobi, jun`

### 예외 처리 구현 리스트

- [ ] 구분자가 쉼표가 아닐 경우 예외 처리
- [ ] 자동차 이름이 두 개이상이 아닐 경우 예외 처리
- [ ] 자동차 이름이 5자 이하가 아닐 경우 예외 처리
- [ ] 자동차 이동 시도 횟수가 0 이상 양수가 아닐 경우 예외 처리