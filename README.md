...
- [x] | 1. 복수 우승자가 정상적으로 선정되는지 테스트
- [x] | 2. 입력된 횟수만큼 정확히 게임이 진행되는지 테스트
- [x] | 3. `자동차` 이름의 양끝단의 공백 제거 테스트

*예외 테스트*

- [x] | 1. `자동차` 이름이 비어있는 경우 예외 발생 테스트
- [x] | 2. `자동차` 이름이 중복될 경우 예외 발생 테스트
- [x] | 3. `자동차` 이름에 공백만 입력된 경우 예외 발생 테스트
- [x] | 4. `자동차` 이름이 비어있는 경우 예외 발생 테스트
- [x] | 5. 시도 횟수가 1 이상의 int 값이 아닐 경우 예외 발생 테스트
