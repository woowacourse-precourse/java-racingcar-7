...
# 구현할 기능 목록
- Car 클래스
    - 자동차 이름을 저장할 수 있어야함 -> `String name`
    - 자동차의 현재 위치를 저장할 수 있어야함 -> `int location`
    - 자동차를 전진시키는 로직 -> `go()`
        - 전진하는 조건은 0에서 9 사이에서 무작위 값을 구한 후 무작위 값이 4 이상일 경우이다.
- 입력을 받는 기능 -> `RacingView.input()`
    - `RacingView.inputValidate()`
...
- 출력을 하는 기능 -> `RacingView.printWinner()`
    - 우승자 소개 기능
        - 자동차 경주 게임을 완료한 후 누가 우승했는지를 알려준다. 우승자는 한 명 이상일 수 있다.
        - 우승자가 여러 명일 경우 쉼표(,)를 이용하여 구분한다.
- 우승자를 저장하는 기능 -> `RacingUtils.getWinnerCarList()`
  - 각 차마다 위치를 저장하고 레이스가 종료된 후 가장 멀리간 차를 뽑아 출력하는 로직으로 구현 
