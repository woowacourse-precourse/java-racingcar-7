...
- [x] **사용자에게 경주할 자동차 이름을 입력받는다.**
    ...
    - [ ] 입력된 각 자동차 이름의 앞뒤 공백을 제거하고 이름이 빈 문자열(`""`)인 자동차가 있는지 확인한다.
        - [ ] `이름이 빈 문자인 자동차가 있는` 경우 `IllegalArgumentException`을 발생시키고 **"`자동차 이름은 빈 문자열일 수 없습니다.`"** 라는 오류 메시지를 출력한 후 프로그램을 종료한다.
