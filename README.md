...
### 경주
#### Racing
- [x] 처음 이동 횟수(스테이지수)를 기록한다.
- [x] 이동 횟수(스테이지수)를 차감한다.
- [x] 이동 횟수(스테이지수)에 따라 경기 진행 및 종료 여부를 판단한다.
#### RacingResult
- [x] 우승 자동차를 결정한다.
- [x] 우승 자동차를 반환한다.
...
#### OutputView
- [x] 경주 시작 메시지를 출력한다.
- [x] 각 이동 단계마다 이동 결과를 출력한다.
- [x] 최종 우승 자동차를 출력한다.

### Controller
#### RacingController (운영자)
- [x] 경주를 각 단계(횟수)에 따라 실행한다.
- [x] 경주의 흐름 및 상태를 정리한다.
- [x] 어시스턴트에게 자동차 이동을 요청한다.
...
- [x] InputCarNamesValidator
    ...
    - [x] 공백이 입력되었는지 확인한다.
- [x] InputCountValidator
    - [x] 이동 횟수(스테이지수)가 1 이상의 숫자인지 검증한다.
    - [x] 숫자가 아닌 값이 입력되었는지 확인한다.
    - [x] 입력한 숫자의 자료형 범위를 확인한다.
    - [x] 공백이 입력되었는지 확인한다.
-- --
## 구현한 기능 Test 목록
입력
- [x] 자동차 이름을 입력 받습니다.
- [x] 이동 횟수(스테이지수)를 입력 받습니다.

출력
- [x] 경주 시작시 주어진 포맷에 맞춰 "출력 결과" string을 출력한다.
- [x] 첫번째 이동 후에 진행 결과 출력을 테스트한다.
- [ ] 연속된 이동 후에 진행 결과 출력을 테스트한다.

Car
- [x] 생성자를 통해 자동차 객체 생성시 올바르게 생성되는지 테스트합니다.
- [x] 자동차가 IntValue값에 맞게 이동하는지 테스트합니다.

Cars
- [x] Cars List를 올바르게 생성하는지 테스트한다.
...
RacingResult
- [x] 한 명이 우승자일 때 한 명의 우승자만 특정지어 올바르게 판단해서 우승자를 반환하는지 테스트한다.
- [x] 여러 명이 우승일 때 여러 명의 우승자를 올바르게 판단해서 우승자를 반환하는지 테스트한다.

InputCarNamesValidator
- [x] 자동차 이름이 5자 이하일 때, 올바르게 검증하는지 테스트한다.
- [x] 자동차 이름이 중복 입력되었을때, 올바르게 검증하는지 테스트한다.
- [x] 공백이 입력되었을 때, 올바르게 검증하는지 테스트한다.

InputCountValidator
- [ ] 이동 횟수(스테이지수)가 1 이상의 숫자일 때, 올바르게 검증하는지 테스트한다.
- [ ] 숫자가 아닌 값이 입력되었을 때, 올바르게 검증하는지 테스트한다.
- [ ] 입력한 숫자의 자료형 범위를 넘었을 때, 올바르게 검증하는지 테스트한다.
- [ ] 공백이 입력되었을 때, 올바르게 검증하는지 테스트한다.
...
#### 추가 단어 설명
- 스테이지수와 이동 횟수는 동일하게 생각했습니다. 이동 횟수에 따라서 이동 횟수를 차감하면서 자동차는 이동할 기회를 갖지만, 이동 후에 표현이 애매해서 스테이지와 이동 횟수를 혼용하여 문서화하였습니다.
#### 협력 목표:
- n개의 자동차가 k번의 이동 횟수(스테이지)안에서 가장 멀리 이동한 자동차를 가려내는 것
...
12. 운영자는 경기기록자에게 이동 횟수(스테이지수)를 차감하라고 말한다.
13. 경기기록자는 이동 횟수(스테이지수)를 차감한다.
14. 운영자는 경기기록자에게 경기 진행 여부를 물어본다.
15. 경기기록자는 이동 횟수(스테이지수)가 남았는지 확인한다.
...
- 운영자가 이동 횟수(스테이지수)를 차감해달라고 경기기록자에게 요청
    - 경기기록자는 이동 횟수(스테이지수)를 차감함
- 운영자는 경기 진행 여부를 경기기록자에게 요청
    - 경기기록자는 이동 횟수(스테이지수)에 따라 경기 진행(반복) 및 경기 종료(반복 종료)를 판단
...
- 하는 것:
    ...
    - 이동 횟수(스테이지수) 차감을 요청한다.
    - 경기를 진행하거나 종료한다.

**경기기록자**
- 하는 것:
    - 이동 횟수(스테이지수)를 기록한다.
    - 이동 횟수(스테이지수)에 따른 경기 진행 여부 판단한다.
- 아는 것:
    - 남은 이동 횟수(스테이지수)
    - 경기 진행 여부
...
#### Racing (경기기록자)
- 초기 이동 횟수(스테이지수)를 기록한다.
- 이동 횟수(스테이지수)를 차감한다.
- 이동 횟수(스테이지수)에 따라 경기 진행 및 종료 여부를 판단한다.
...
- InputCarNamesValidator
    ...
    - 공백이 입력되었는지 확인한다.
- InputCountValidator
    - 이동 횟수(스테이지수)가 1 이상의 숫자인지 검증한다.
    - 숫자가 아닌 값이 입력되었는지 확인한다.
