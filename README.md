...
1. **정상적인 자동차 이름 입력**
   - 입력: `"pobi, jun, car1"`
   - 기대 결과:
      - 자동차 객체 리스트에 포함: `["pobi", "jun", "car1"]`

2. **공백이 포함된 이름 입력 처리**
   - 입력: `" pobi ,  jun , car2 "`
   - 기대 결과:
      - `trim()` 후 처리된 리스트: `["pobi", "jun", "car2"]`

3. **숫자와 특수문자가 포함된 이름 입력**
   - 입력: `"car1!, pobi@"`
   - 기대 결과:
      - 리스트: `["car1!", "pobi@"]`

4. **정확히 5자로 이루어진 이름 입력**
   - 입력: `"abcde"`
   - 기대 결과:
      - 객체 생성 성공: `"abcde"`
...
2. **이름이 5자를 초과하는 경우**
   - 입력: `"pobi, longname"`
   - 예외 메시지: `"Error: 자동차 이름은 5자 이하로 입력해야 합니다."`

3. **빈 문자열 입력 시 예외 발생**
   - 입력: `"pobi, "`
   - 예외 메시지: `"Error: 자동차 이름은 비어있을 수 없습니다."`
...
5. **`null` 입력 시 예외 발생**
   - 입력: `null`
   - 예외 메시지: `"Error: 입력값은 비어있을 수 없습니다."`

6. **숫자만으로 이루어진 이름이 5자를 초과할 경우**
   - 입력: `"123456, pobi"`
   - 예외 메시지: `"Error: 자동차 이름은 5자 이하로 입력해야 합니다."`
...
1. **`null` 입력 시 예외 발생**
   - 입력: `null`
   - 예외 메시지: `"Error: 입력값이 null일 수 없습니다."`

2. **문자 입력 시 예외 발생**
   - 입력: `"a"`
   - 예외 메시지: `"Error: 시도 횟수는 1 이상 2,147,483,647 이하의 정수여야 합니다."`

3. **음수 입력 시 예외 발생**
   - 입력: `"-1"`
   - 예외 메시지: `"Error: 시도 횟수는 1 이상의 값이어야 합니다."`

4. **`int` 범위를 초과하는 숫자 입력 시 예외 발생**
   - 입력: `"21474836481"`
   - 예외 메시지: `"Error: 시도 횟수는 1 이상 2,147,483,647 이하의 정수여야 합니다."`
...
### **Happy Case**
1. **랜덤 값이 4 이상인 경우 이동 가능**
   - **Stub 값**: `4, 5, 6, 7, 8, 9`
   - 기대 결과: 이동 조건 충족

2. **랜덤 값이 3 이하인 경우 이동 불가**
   - **Stub 값**: `0, 1, 2, 3`
   - 기대 결과: 이동 조건 불충족
...
1. **랜덤 값이 0~9 범위를 벗어난 경우**
   - **Stub 값**: `-1, 10`
   - 예외 메시지: `"Error: 무작위 값은 0에서 9 사이여야 합니다."`
...
### **Happy Case**
1. **자동차 이름과 시도 횟수 입력 정상 수신**
   - 입력: `"pobi, jun, car1\n5\n"`
   - 기대 결과:
      - 자동차 이름: `"pobi, jun, car1"`
      - 시도 횟수: `"5"`

2. **자동차 이름 입력 안내 메시지 출력**
   - 기대 결과: `"경주할 자동차 이름을 입력하세요.(이름은 쉼표(,) 기준으로 구분)"`

3. **시도 횟수 입력 안내 메시지 출력**
   - 기대 결과: `"시도할 횟수는 몇 회인가요?"`
